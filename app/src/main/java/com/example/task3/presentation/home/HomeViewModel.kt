...
class HomeViewModel @Inject constructor(private val useCase: NewsApiUseCase) : ViewModel() {
    ...
    fun fetchHeadlines(query: String = "us") =
        useCase.getHeadlines(mapOf("country" to query)).fetchData(_resHeadlines)

    private val _resNews = MutableLiveData<HomeUIState>()
    val resNews: LiveData<HomeUIState> get() = _resNews

    fun fetchNews() = useCase.getEverything(mapOf("q" to "tesla")).fetchData(_resNews)

    fun fetchNews(query: String) =
        useCase.getEverything(mapOf("q" to query), 200).fetchData(_resNews, 300)

    private fun Flow<ResourceState<NewsDTO>>.fetchData(
        livedata: MutableLiveData<HomeUIState>, debounce: Long = 0) =
